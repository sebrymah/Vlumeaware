...
function safeUrl(raw: string): string | null {
  ...
  if (/^(javascript|vbscript|file):/i.test(decoded)) return null;
  if (/^data:/i.test(decoded) && !/^data:image\//i.test(decoded)) return null; // only inline images
  if (/^(https?:\/\/|mailto:|data:image\/|\/|#)/i.test(v)) return v;
  if (!/^[a-z][a-z0-9+.-]*:/i.test(v)) return v; // bare relative path
  return null;
}
...
export function sanitizeLandingHtml(input: string | null | undefined): string {
  ...
  // so scrub the block body for @import / expression() / script URLs here.
